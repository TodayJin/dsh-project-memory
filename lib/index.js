...
function apply(ctx, config) {
	...
	ctx.on("agent/pre-step", async ({ agent }, next) => {
		...
		if (state.digest === digest && (visible === "present" || (visible === "unknown" && state.injected))) {
			return downstream;
		}
		state.injected = true;
		state.digest = digest;
		...
		// Swap the published block in place, then shrink whatever earlier copies this
		// session accumulated before the plugin could replace them. Only the first
		// injection of a session — or one after the surface compacted it away — appends.
		if (published !== undefined && published.length > 0) {
			if (replaceInSession(agent, block, published.at(-1))) {
				collapseEarlierBlocks(agent, published.slice(0, -1));
				return downstream;
			}
		}
		return { ...downstream, messages: [...downstream.messages, block] };
	});
