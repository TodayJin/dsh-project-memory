/**
 * dsh-project-memory — browser half.
 *
...
`;
		 ...
		 * @param path - path below /api/project-memory.
		...
		function SettingsPanel() {
			...
			const forgetWorkspace = async () => {
				...
				setBusy("forget");
				try {
					await call("/forget", { method: "POST", body: { root: selected } });
					const list = await loadWorkspaces();
					const next = list.length > 0 ? list[0].root : "";
					setSelected(next);
					if (next.length > 0) await loadFiles(next);
					else setFiles(null);
					setStatus({ kind: "ok", text: "已从列表移除（磁盘上的文件未动）" });
				} catch (failure) {
					setStatus({ kind: "err", text: messageOf(failure) });
				}
				setBusy("");
				...
				children.push(
					h(
						...
						h(
							...
							h("button", { className: "dsh-pm-btn dsh-pm-btn-danger", onClick: forgetWorkspace, disabled: busy !== "" }, busy === "forget" ? "移除中…" : "从列表移除"),
						),
					),
					h(
						"div",
						...
						h(
							...
							h(
								...
								["PROJECT.md", "DECISIONS.md", "SESSIONS.md"].map((fileName) =>
									h(
										"button",
		...
		function apply(ctx) {
			...
			style.id = "dsh-project-memory-settings-style";
		...
		exports.name = "dsh-project-memory";
