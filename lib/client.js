...
.dsh-pm-stale{display:flex;gap:8px;align-items:baseline;border:1px solid #c47f1755;background:#c47f1712;border-radius:8px;padding:8px 10px;font-size:12px;line-height:1.5}
.dsh-pm-stale .dsh-pm-badge{color:#c47f17;border-color:currentColor;font-size:11px}
		...
		function SettingsPanel() {
			...
			if (current !== null) {
				...
				children.push(
						...
						writable
							? h(
									...
									editing
										...
										: h("button", { className: "dsh-pm-btn dsh-pm-btn-primary", onClick: () => { setEditingInstruction(false); setDraft(body); setEditing(true); setStatus({ kind: "idle", text: "" }); }, disabled: busy !== "" }, "编辑"),
									editing
							...
							: h(
									"div",
									{ className: "dsh-pm-row" },
									h("span", { className: "dsh-pm-hint" }, tab === "__overview__" ? "概览是只读的。" : "归档是只读的；要改就用「恢复这条」搬回活日志。"),
									h("button", { className: "dsh-pm-btn", onClick: () => loadFiles(selected), disabled: busy !== "" }, "重新读取"),
								),
						h(
							"div",
							{ className: "dsh-pm-row" },
							h("button", { className: "dsh-pm-btn", onClick: exportMemory, disabled: busy !== "" }, busy === "export" ? "导出中…" : "导出这组记忆"),
							h("button", { className: "dsh-pm-btn", onClick: () => fileInput.current?.click(), disabled: busy !== "" }, busy === "import" ? "导入中…" : "导入记忆包"),
							h("input", {
								ref: fileInput,
								type: "file",
							...
							h("span", { className: "dsh-pm-hint" }, "导出为 JSON；导入会覆盖同名文件"),
						),
						h(
							"div",
					...
					h(
						"div",
						{ key: "boot", className: "dsh-pm-panel dsh-pm-panel-quiet" },
						h(
							"div",
							{ className: "dsh-pm-row" },
							h("span", { className: "dsh-pm-panel-label" }, "项目指令文件"),
